...
public class Renderer {
    ...
    public Renderer(GameContainer gc) {
        pW = gc.getWIDTH();
        pH = gc.getHEIGHT();
        p = ((DataBufferInt) gc.getWindow().getBfImage().getRaster().getDataBuffer()).getData();
        zb = new int[p.length];
        lm = new int[p.length];
        lb = new int[p.length];
    }
    ...
    public void process() {
        ...
        for (int i = 0; i < lightRequests.size(); i++) {
            ...
            this.drawLightRequest(l.getLight(), l.getLocX(), l.getLocY());
        }
    ...
    public void drawImageTile(ImageTile image, int offX, int offY, int tileX, int tileY) {
        ...
        for (int y = newY; y < newHeight; y++) {
            for (int x = newX; x < newWidth; x++) {
                ...
                if ((((image.getP()[(x + tileX * image.getTileW())
                        + (y + tileY * image.getTileH()) * image.getW()]) >> 24) & 0xff) != 0){
                    setLightBlock(x + offX, y + offY, image.getLightBlock());
                }
                setLightBlock(x + offX, y + offY, image.getLightBlock());
            }
